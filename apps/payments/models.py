...
class EscrowAccount(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} Wallet - {format_uzs(self.balance)}"
...
class WithdrawalRequest(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            ...
            if old_instance.status == self.Status.PENDING and self.status == self.Status.APPROVED:
                wallet = self.user.escrow_wallet
                if wallet.balance >= self.amount:
                    wallet.balance -= self.amount
                    wallet.save()
                else:
                    raise ValidationError("Foydalanuvchi hisobida mablag' yetarli emas!")
        super().save(*args, **kwargs)
